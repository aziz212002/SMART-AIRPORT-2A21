...
bool vols::modifiervols(int num_vols,QDate d_vols,int cap_vols,QString dest ,int nb)
{

        QSqlQuery query;
        query.prepare("update vols set d_vols=:d_vols,cap_vols=:cap_vols,dest=:dest,nb=:nb where num_vols=:num_vols");
        query.bindValue(":num_vols",num_vols);
        query.bindValue(":d_vols",d_vols);
        query.bindValue(":cap_vols",cap_vols);
        query.bindValue(":dest",dest);
